...
static bool sense_voice_params_parse(int argc, char **argv, sense_voice_params &params) {
    for (int i = 0; i < argc; i++) {
        ...
        // vad parameters
        else if (arg == "-vt" || arg == "--threshold") {
            params.threshold = std::stof(argv[++i]);
        } else if (arg == "-vnt" || arg == "--neg_threshold") {
            params.neg_threshold = std::stof(argv[++i]);
        } else if (arg == "--min_speech_duration_ms") {
            params.min_speech_duration_ms = std::stoi(argv[++i]);
        } else if (arg == "--max_speech_duration_ms") {
            params.max_speech_duration_ms = std::stoi(argv[++i]);
        } else if (arg == "--min_silence_duration_ms") {
            params.min_silence_duration_ms = std::stoi(argv[++i]);
        } else if (arg == "--speech_pad_ms") {
            params.speech_pad_ms = std::stoi(argv[++i]);
        }
...
static void gen_argv(const char *str_params, int *argc, char ***argv) {
    // 计算参数数量
    int count = 0;
    const char *temp = str_params;
    while (*temp) {
    ...
    *argv = (char **) malloc((count + 1) * sizeof(char *));
    ...
    for (int i = 0; i < count; i++) {
        ...
        strncpy((*argv)[i], start, length);
        (*argv)[i][length] = '\0';
    }
    (*argv)[count] = NULL;// 最后一个元素为 NULL
}

static int parse_params(sense_voice_params &params, const char *str_params) {
    int argc;
    char **argv;
    gen_argv(str_params, &argc, &argv);
    sense_voice_params_parse(argc, argv, params);
    for (int i = 0; i < argc; i++) {
        free(argv[i]);// 释放内存
    }
    free(argv);// 释放内存
...

        {
...

            {
                ...
                ctx->state->vad_lstm_context_buffer = ggml_backend_alloc_buffer(ctx->state->backends[0],
                                                                                ggml_nbytes(ctx->state->vad_lstm_context) + ggml_backend_get_alignment(ctx->state->backends[0]));
                ctx->state->vad_lstm_hidden_state_buffer = ggml_backend_alloc_buffer(ctx->state->backends[0],
                                                                                     ggml_nbytes(ctx->state->vad_lstm_hidden_state) + ggml_backend_get_alignment(ctx->state->backends[0]));
